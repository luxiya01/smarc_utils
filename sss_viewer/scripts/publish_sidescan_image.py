...
class SidescanPublisher:
    ...
    corresponding waterfall image to /vehicle/payload/sidescan/image/
    and /vehicle/payload/sidescan/image/compressed"""
    ...
    def __init__(self, width_in=1000, width_out=256, height=1000):
        ...
        self.sidescan_sub = rospy.Subscriber("/sam/payload/sidescan", Sidescan,
                self.callback)
        self.sidescan_image_pub = rospy.Publisher("/sam/payload/sidescan/image", Image, queue_size=2)
        self.sidescan_image_compressed_pub = rospy.Publisher("/sam/payload/sidescan/image/compressed", CompressedImage, queue_size=2)
    ...
    def publish_image(self):
        ...
        try:
            imgmsg = self.bridge.cv2_to_imgmsg(resized_img)
            self.sidescan_image_pub.publish(imgmsg)
            compressed_imgmsg = self.bridge.cv2_to_compressed_imgmsg(resized_img)
            self.sidescan_image_compressed_pub.publish(compressed_imgmsg)
        except CvBridgeError as e:
            print(f"Error converting numpy image to imgmsg: {e}")
